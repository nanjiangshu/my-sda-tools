...
setup_sda_kubeconfig() {
    ...
    # 2. macOS Compatibility (The clean way)
    ...
    (
        ...
        if [[ -f "kubeconfig.yaml" ]]; then
            ...
            # 4. Fix the SSL Certificate mismatch (the x509 error)
            # This handles the 'ingress.local' vs 'bp-v2-k8s' mismatch automatically
            kubectl config set-cluster "default-cluster" --insecure-skip-tls-verify=true --kubeconfig="$KUBECONFIG_FILE" >/dev/null 2>&1
        else
